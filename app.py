...
# సింపుల్ బ్యాక్‌గ్రౌండ్ డిజైన్ (CSS) - ఇక్కడ ఎర్రర్ సరిచేయబడింది (unsafe_allow_html=True)
st.markdown("""
    <style>
...
st.markdown("---")
st.subheader("😂 ネటి నవ్వుల తోట (Stress Relief Joke)")
st.info("""
...
if uploaded_file is not None:
    ...
    # ఇది ఒక ఉదాహరణ అనాలిసిస్ మోడల్.
    file_name = uploaded_file.name.lower()
    if "bonda" in file_name or "sweet" in file_name or "fry" in file_name or "బజ్జీ" in file_name:
...
if st.button("📊 రోజువారీ రిపోర్ట్ (Everyday Report) జనరేట్ చేయండి"):
    ...
    for time_slot, data in st.session_state.tracker.items():
        ...
        <br>
        """, unsafe_allow_html=True) # ఇక్కడ కూడా ఎర్రర్ సరిచేయబడింది
